 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class behat_theme_solent2019_behat_deprecated extends behat_deprecated {
    ...
    public function i_navigate_to_node_in($nodetext, $parentnodes) {
        ...
        if ($firstnode === get_string('administrationsite')) {
            $this->execute('behat_theme_solent2019_behat_navigation::i_select_from_flat_navigation_drawer',
                    array(get_string('administrationsite')));
            $this->execute('behat_theme_solent2019_behat_navigation::select_on_administration_page', array($nodelist));
            return;
        }
        ...
        if ($firstnode === get_string('sitepages')) {
            if ($nodetext === get_string('calendar', 'calendar')) {
                ...
                $this->execute('behat_theme_solent2019_behat_navigation::select_node_in_navigation',
        ...
        if ($firstnode === get_string('courseadministration')) {
            ...
            $this->execute('behat_theme_solent2019_behat_navigation::go_to_main_course_page', array());
        }

        $this->execute('behat_theme_solent2019_behat_navigation::select_from_administration_menu', array($nodelist));
    }
}
